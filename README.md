...
## **Docker Daemon**
    ...
    - Docker Daemon là cái thực sự thực hiện lệnh được gửi đến Docker Client — như xây dựng, chạy, và phân phối các container của bạn. Docker Daemon chạy trên máy tính máy chủ, nhưng như một người sử dụng, bạn không bao giờ tương tác trực tiếp với Daemon. Docker Client cũng có thể chạy trên máy tính máy chủ, nhưng điều này không bắt buộc. Nó có thể chạy trên một máy tính khác và tương tác với Docker Daemon đang chạy trên máy tính máy chủ.

## **Volumes**

    - Volumes là "dữ liệu" của một container, được khởi tạo khi một container được tạo. Volumes cho phép bạn lưu trữ và chia sẻ dữ liệu của container. Dữ liệu volumes là riêng biệt với Hệ thống tệp hợp mặc định và tồn tại dưới dạng thư mục và tệp thông thường trên hệ thống tệp của máy chủ. Vì vậy, ngay cả khi bạn xóa, cập nhật hoặc xây dựng lại

container, volumes dữ liệu sẽ vẫn giữ nguyên. Khi bạn muốn cập nhật một volume, bạn thay đổi trực tiếp vào nó. (Như một điều thêm vào, volumes dữ liệu có thể được chia sẻ và sử dụng lại giữa nhiều container, điều này khá tuyệt vời.)
